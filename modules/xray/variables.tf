...
locals {
  ...
  sls_lambda_role_name = "${local.sls_lambda_role_name != "" ? local.sls_lambda_role_name : local.opt_create_lambda_role != "" ? aws_iam_role.lambda.name : local.sls_lambda_default_role_name}"

  # The calculated Lambda role ARN. Precedence is custom name > created by module > default Serverless.
  sls_lambda_role_arn = "${local.sls_lambda_custom_role_arn != "" ? local.sls_lambda_custom_role_arn : local.opt_create_lambda_role != "" ? aws_iam_role.lambda.arn : local.sls_lambda_default_role_arn}"
